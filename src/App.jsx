...
const pages = {
  ...
  arena: StudyArenaPage,
  chemistry: StudyArenaPage,
...
export default function App() {
  ...
  useEffect(() => { useStore.getState().recompute(); }, []);
  useEffect(() => {
      ...
      <div className="flex min-h-screen">
        ...
        {sidebarOpen && (
          <div className="fixed inset-0 z-40 lg:hidden" onClick={() => useStore.getState().setSidebarOpen(false)}>
            ...
            <div className="absolute left-0 top-0 h-full w-[280px] bg-white dark:bg-surface-dark shadow-2xl animate-slideIn z-50"><Sidebar mobile /></div>
          </div>
        )}
        <div className={`flex-1 flex flex-col min-w-0 min-h-screen transition-[margin] duration-200 ${desktopOffset}`}>
          <Header />
          <main className="flex-1 px-4 pb-28 md:px-6 lg:px-8 xl:px-10 pt-4 max-w-[1600px] w-full mx-auto"><PageComponent /></main>
        </div>
      </div>
      <MobileNav />
