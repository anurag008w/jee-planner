...
import {
  ...
  BarChart3, List, Settings2, ChevronRight,
} from 'lucide-react';
...
const navItems = [
  ...
  { id: 'settings', label: 'Settings', icon: Settings2, shortLabel: 'Settings' },
];
    ...
    <aside className={`${mobile ? 'flex h-full' : desktopVisible ? 'hidden lg:flex' : 'hidden'} fixed top-0 left-0 h-screen w-[var(--sidebar-width)] flex-col bg-white dark:bg-[#1a1c2b] border-r border-[var(--color-border)] dark:border-[#2c2f40] z-50 transition-all duration-200`}>
      <div className="p-5 border-b border-[var(--color-border)] dark:border-[#2c2f40]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-[15px] text-gray-900 dark:text-white leading-tight">Lecture Planner</h1>
            <p className="text-[11px] text-gray-500 dark:text-gray-400 font-medium">JEE 2027 • 392 Lectures</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto scrollbar-thin" aria-label="Pages">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => { setPage(item.id); if (mobile) setSidebarOpen(false); }} aria-current={active ? 'page' : undefined}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-[13.5px] font-medium transition-all duration-200 group relative ${active ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white'}`}>
              {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-indigo-600 dark:bg-indigo-400 rounded-r-full" />}
      ...
      <div className="p-3 border-t border-[var(--color-border)] dark:border-[#2c2f40]">
        <p className="px-3.5 py-2 text-[10.5px] text-gray-400 dark:text-gray-600">Start: 11 Sep 2026 • End: 25 Dec 2026</p>
      </div>
    </aside>
  );
}
