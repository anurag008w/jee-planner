...
const enforceLectureOrder = (schedule, lectures, completions) => {
  ...
  Object.entries(schedule.resolved).forEach(([id, meta]) => {
    ...
    const item = { ...lecture, phase: oldItem?.phase || lecture.phase, isBacklog: meta.isBacklog, resolvedDate: meta.resolvedDate };
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        toggleComplete: (id) => { set((s) => ({ completions: { ...s.completions, [id]: s.completions[id] === 'completed' ? 'not_started' : 'completed' } })); get().recompute({ freezeToday: true }); },
        completeMany: (ids) => { set((s) => { const completions = { ...s.completions }; ids.forEach((id) => { completions[id] = 'completed'; }); return { completions }; }); get().recompute({ freezeToday: true }); },
        toggleOffDay: (date) => {
          set((s) => { const has = s.settings.offDays.includes(date); const offDays = has ? s.settings.offDays.filter((d) => d !== date) : [...s.settings.offDays, date].sort(); return { settings: { ...s.settings, offDays } }; });
          get().recompute();
        },
        setSundaysOff: (on) => {
          set((s) => { const offDays = on ? [...new Set([...s.settings.offDays, ...defaultOffDays])].sort() : s.settings.offDays.filter((d) => new Date(d).getDay() !== 0); return { settings: { ...s.settings, offDays } }; });
          get().recompute();
        },
        setAllCommonHolidays: (on) => {
          set((s) => { const offDays = on ? [...new Set([...s.settings.offDays, ...commonHolidayDates])].sort() : s.settings.offDays.filter((d) => !commonHolidayDates.includes(d)); return { settings: { ...s.settings, offDays } }; });
          get().recompute();
        },
        ...
        setChapterPhase: (chapter, phase) => { set((s) => { const cp = { ...(s.settings.chapterPhases || {}) }; if (phase === '' || phase === 'auto') delete cp[chapter]; else cp[chapter] = phase; return { settings: { ...s.settings, chapterPhases: cp } }; }); get().recompute(); },
