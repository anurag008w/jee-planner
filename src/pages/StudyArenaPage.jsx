...
import { Atom, Calculator, FlaskConical, CheckCircle2, BookOpen, Clock3, ChevronDown, ChevronUp, X } from 'lucide-react';
...
export default function StudyArenaPage() {
  ...
  const defaultSubject = SUBJECTS.some(s => s.key === preferred) ? preferred : legacyChemistryEntry ? 'Chemistry' : 'Physics';
  ...
  const branchStats = useMemo(() => {
    ...
    return CHEM_TABS.slice(1).map(tab => {
      ...
      return { tab, total: branchLectures.length, done, pct: branchLectures.length ? Math.round((done / branchLectures.length) * 100) : 0, color: getBranchColor(branch) };
    });
  ...
  return (
    <div className="animate-fadeIn space-y-5 pb-8">
      <header className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${style.icon} text-white flex items-center justify-center shadow-lg`}><SubjectIcon size={20} /></div>
      ...
      <div className="grid grid-cols-3 gap-2 rounded-2xl border border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-white/[.025] p-1.5">
        {SUBJECTS.map(s => {
          ...
          return (
            <button key={s.key} onClick={() => changeSubject(s.key)} className={`min-w-0 rounded-xl px-2 py-2.5 transition-all duration-200 border ${active ? `${sStyle.active} shadow-sm` : 'border-transparent text-gray-500 dark:text-gray-400 hover:bg-white dark:hover:bg-white/[.04] hover:text-gray-800 dark:hover:text-gray-200'}`}>
              <span className="flex items-center justify-center gap-1.5 min-w-0"><Icon size={15} className="flex-shrink-0" /><span className="text-[11.5px] sm:text-[12px] font-semibold truncate">{s.label}</span><span className="hidden sm:inline text-[9.5px] opacity-50 flex-shrink-0">{total}L</span></span>
            </button>
          );
        })}
      </div>
      ...
      {subject === 'Chemistry' && (
        <div className="flex gap-2 overflow-x-auto scrollbar-thin pb-1 -mx-1 px-1">
          {CHEM_TABS.map(tab => (
            <button key={tab} onClick={() => { setChemTab(tab); setExpanded(null); }} className={`px-4 py-2 rounded-xl text-[12px] whitespace-nowrap font-medium transition-all border ${chemTab === tab ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800/30 shadow-sm' : 'bg-white dark:bg-[#1a1c2b] text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 border-gray-100 dark:border-white/5'}`}>{tab}</button>
          ))}
        </div>
      )}

      <section className={`bg-gradient-to-br ${style.hero} rounded-2xl p-5 text-white shadow-lg`}>
        <div className="flex items-center justify-between mb-3 gap-4"><div><p className="text-white/70 text-[12px] font-semibold uppercase tracking-wider">Overall {subject}</p><p className="text-[11px] text-white/55 mt-0.5">Keep the sequence moving, one chapter at a time.</p></div><p className="text-2xl font-bold flex-shrink-0">{data.pct}%</p></div>
        <div className="w-full h-2 bg-white/15 rounded-full overflow-hidden"><div className="h-full bg-white rounded-full transition-all duration-700" style={{ width: `${data.pct}%` }} /></div>
        <p className="text-white/60 text-[11px] mt-2 text-right">{data.done} of {data.total} completed</p>
      </section>

      {subject === 'Chemistry' && (
        <div className="grid grid-cols-3 gap-3">
          {branchStats.map(branch => <button key={branch.tab} onClick={() => { setChemTab(branch.tab); setExpanded(null); }} className={`text-left rounded-xl p-3 border transition-all hover:shadow-sm ${branch.color.bg} ${branch.color.dark} ${branch.color.text || ''}`}><p className="text-[10.5px] font-semibold uppercase tracking-wider opacity-70">{branch.tab}</p><p className="text-lg font-bold mt-1">{branch.pct}%</p><p className="text-[11px] opacity-60">{branch.done}/{branch.total}</p></button>)}
        </div>
      )}
      ...
      <div className="relative">
        ...
        {data.chapters.map((chapter, idx) => {
          ...
          const branchColor = subject === 'Chemistry' && chapter.lectures[0]?.chemistryBranch ? getBranchColor(chapter.lectures[0].chemistryBranch) : null;
                ...
                <div className={`flex-1 rounded-xl border transition-all duration-200 overflow-hidden ${isOpen ? 'bg-white dark:bg-[#1a1c2b] border-gray-200 dark:border-white/10 shadow-sm' : 'bg-white dark:bg-[#1a1c2b] border-gray-100 dark:border-white/5 hover:border-gray-200 dark:hover:border-white/10 hover:shadow-sm'}`}>
                  <button className="w-full text-left p-4" onClick={() => setExpanded(isOpen ? null : chapter.seriesKey)} aria-expanded={isOpen}>
                    <div className="flex items-start justify-between gap-3 mb-2"><div className="min-w-0"><div className="flex items-center gap-2 flex-wrap mb-1.5">
                      {branchColor && <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${branchColor.badge} ${branchColor.dark}`}>{chapter.lectures[0].chemistryBranch.replace(' Chemistry', '')}</span>}
                    ...
                    {pairing && <div className="mt-2 pt-2 border-t border-gray-100 dark:border-white/5 flex items-start gap-1.5 text-[11px]"><span className="text-gray-400 dark:text-gray-500 font-semibold whitespace-nowrap">11th base:</span><span className="text-gray-600 dark:text-gray-300 leading-snug truncate">{pairing.base}</span></div>}
                  </button>
