...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-8">
      ...
      <section className="card-surface p-5 md:p-6"><SectionTitle icon={CalendarDays} title="Schedule" subtitle="Control how your lecture plan is resolved day by day."/><div className="mt-5 space-y-4">
        <Row title="Schedule start date" desc="Day 1 se poora lecture plan recalculate hoga. Lectures/topics unchanged rahenge."><div className="flex items-center gap-2"><input type="date" value={settings.startDate || ''} onChange={e=>setStartDate(e.target.value)} className="field w-auto"/><button className="btn btn-secondary px-3 py-2 text-xs" onClick={()=>setStartDate('2026-09-11')}><RotateCcw size={13}/> Reset</button></div></Row>
        <Row title="Preview date" desc="Testing ke liye kisi bhi date par backlog behaviour simulate karo."><div className="flex items-center gap-2"><input type="date" value={settings.previewDate || ''} onChange={e=>setPreviewDate(e.target.value)} className="field w-auto"/>{settings.previewDate&&<button className="btn btn-ghost px-2 py-2 text-xs" onClick={()=>setPreviewDate('')}>Clear</button>}</div></Row>
      ...
      <section className="card-surface p-5 md:p-6"><SectionTitle icon={CalendarDays} title="Holidays" subtitle="Per-holiday control, plus one-tap bulk action."/><div className="flex justify-end mt-4"><button onClick={()=>setAllCommonHolidays(!allHolidaysOff)} className="btn btn-secondary px-3 py-2 text-xs">{allHolidaysOff?'Make all study days':'Make all off days'}</button></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 mt-3">{commonHolidays.map(h=>{const off=settings.offDays.includes(h.date);return <div key={h.date} className={`flex items-center justify-between p-3 rounded-xl border ${off?'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800/30':'bg-gray-50 dark:bg-white/[.03] border-gray-200 dark:border-white/10'}`}><div><div className="text-xs font-semibold">{formatDateShort(h.date)}</div><div className="text-[10px] text-gray-400">{off?'Off day':'Study day'}</div></div><MiniSwitch on={off} onClick={()=>toggleOffDay(h.date)}/></div>})}</div></section>
