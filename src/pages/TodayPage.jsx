    ...
    <div className="space-y-6 pb-4">
      ...
      {showSettings && (
        <div className="bg-white dark:bg-[#1a1c2b] rounded-2xl border border-gray-100 dark:border-white/5 p-4 animate-scaleIn space-y-3">
          ...
          <div className="pt-3 border-t border-gray-100 dark:border-white/5">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                ...
                <p className="text-[11px] text-gray-400 dark:text-gray-500">Har holiday me toggle hai — OFF ho toh off-day, ON karo toh study day ban jaata hai.</p>
              </div>
              {(() => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {commonHolidays.map(h => {
                ...
                return (
                  <div key={h.date} className={`flex items-center justify-between gap-2 px-3 py-2 rounded-xl border transition-colors ${isOff ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800/40' : 'bg-white dark:bg-[#1a1c2b] border-gray-100 dark:border-white/10'}`}>
                    <div className="min-w-0">
                      ...
                      <p className={`text-[10px] ${isOff ? 'text-amber-600/70 dark:text-amber-400/70' : 'text-gray-400 dark:text-gray-500'}`}>{isOff ? 'Off Day' : 'Study Day'}</p>
                    </div>
                    <button
          ...
          <div className="pt-3 border-t border-gray-100 dark:border-white/5">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                ...
                <p className="text-[11px] text-gray-400 dark:text-gray-500">Kisi phase ka sirf start+end date bharo — us window me wahi phase chalega (daily capacity wahi). Auto: chapters khud decide karte hain.</p>
              </div>
              <button
                onClick={resetPhaseRanges}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-semibold border transition-all flex-shrink-0 ${settings.phaseRanges ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 hover:bg-amber-100' : 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300'}`}
              >
              ...
              {['Phase 1', 'Phase 2', 'Phase 3', 'Phase 4'].map((p) => {
                ...
                return (
                  <div key={p} className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-colors ${settings.phaseRanges?.[p] ? 'bg-indigo-50/60 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-800/40' : 'bg-white dark:bg-[#1a1c2b] border-gray-100 dark:border-white/10'}`}>
                    <span className={`text-[11px] font-bold px-2 py-1 rounded-lg flex-shrink-0 ${p === 'Phase 1' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : p === 'Phase 2' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : p === 'Phase 3' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>{p.replace('Phase ', 'P')}</span>
                    <input type="date" value={cur.start || ''} onChange={(e) => { const next = { ...(settings.phaseRanges || {}) }; const n = { ...(next[p] || {}), start: e.target.value }; if (!n.start && !n.end) delete next[p]; else next[p] = n; setPhaseRanges(Object.keys(next).length ? next : null); }} className="flex-1 min-w-0 px-2 py-1.5 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-[11.5px] text-gray-700 dark:text-gray-300 outline-none focus:border-indigo-300 dark:focus:border-indigo-600" />
                    <span className="text-[10px] text-gray-400 dark:text-gray-500">to</span>
                    <input type="date" value={cur.end || ''} onChange={(e) => { const next = { ...(settings.phaseRanges || {}) }; const n = { ...(next[p] || {}), end: e.target.value }; if (!n.start && !n.end) delete next[p]; else next[p] = n; setPhaseRanges(Object.keys(next).length ? next : null); }} className="flex-1 min-w-0 px-2 py-1.5 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-[11.5px] text-gray-700 dark:text-gray-300 outline-none focus:border-indigo-300 dark:focus:border-indigo-600" />
                  </div>
                );
              })}
            </div>
          ...
          <div className="pt-3 border-t border-gray-100 dark:border-white/5">
            ...
            <p className="text-[11px] text-gray-400 dark:text-gray-500 mb-3">Progress aur settings is browser ki localStorage me save rehti hai. Extra safety ke liye JSON backup download karo — kisi bhi browser me wapas restore kar sakte ho.</p>
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-semibold bg-indigo-600 hover:bg-indigo-700 text-white transition-colors"><Download size={13} /> Download Backup</button>
              <label className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-semibold bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/15 text-gray-700 dark:text-gray-200 cursor-pointer transition-colors"><Upload size={13} /> Restore Backup<input type="file" accept="application/json,.json" className="hidden" onChange={handleImport} /></label>
              {backupMsg && <span className="text-[12px] font-medium text-emerald-600 dark:text-emerald-400">{backupMsg}</span>}
            </div>
          </div>
        </div>
      )}
      ...
      {!isOffDay && todayPlan.length > 0 && (
        <div className="animate-fadeIn stagger-3">
          <h3 className="text-[14px] font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
            <Calendar size={16} className="text-indigo-500" /> Today's Lectures
            {todayPlan.some(l => l.isBacklog) && <span className="text-[10.5px] font-semibold text-red-500 flex items-center gap-1"><AlertTriangle size={12} /> backlog pehle</span>}
            <span className="ml-auto text-[10.5px] font-bold px-2 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-300 whitespace-nowrap" title="Effort units — ONE SHOT lecture = 0.5">{todayPlan.length} L · {(schedule.dayLoad?.[today] || 0).toFixed(1)}/{schedule.dayCap?.[today] || 0} effort</span>
          </h3>
          <div className="space-y-3">
            {todayPlan.map((lecture, idx) => (
              <div key={lecture.id} className="relative pl-8">
                {idx < todayPlan.length - 1 && <div className="absolute left-[11px] top-12 bottom-0 w-0.5 bg-gray-200 dark:bg-white/10" />}
                <div className={`absolute left-0 top-4 w-6 h-6 rounded-full border-2 flex items-center justify-center text-[10px] font-bold ${completions[lecture.id] === 'completed' ? 'bg-green-500 border-green-500 text-white' : lecture.isBacklog ? 'bg-red-500 border-red-500 text-white' : 'bg-white dark:bg-[#1a1c2b] border-indigo-300 dark:border-indigo-600 text-indigo-600 dark:text-indigo-400'}`}>{lecture.slot}</div>
      ...
      {!isOffDay && todayPlan.length === 0 && (
        <div className="animate-fadeIn stagger-2 text-center py-12">
          <div className="w-20 h-20 mx-auto mb-4 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center"><CheckCircle2 size={32} className="text-indigo-500" /></div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Nothing Scheduled Today</h3>
          <p className="text-[13px] text-gray-500 dark:text-gray-400 max-w-sm mx-auto">{backlog.length > 0 ? `${backlog.length} backlog lecture${backlog.length > 1 ? 's' : ''} pending — clear them from the backlog pool below!` : 'Sab plan ke hisaab se on-track hai. 💪'}</p>
        </div>
      )}
      ...
      {backlog.length > 0 && (
        <div className="animate-fadeIn stagger-4 bg-white dark:bg-[#1a1c2b] rounded-2xl border border-gray-100 dark:border-white/5 overflow-hidden">
          <button onClick={() => setBacklogOpen((o) => !o)} aria-expanded={backlogOpen} aria-label={backlogOpen ? 'Collapse backlog' : 'Expand backlog'} className="w-full px-4 py-3 text-left active:bg-gray-50 dark:active:bg-white/[0.03] transition-colors">
            <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" aria-hidden="true" /><span className="flex-1 min-w-0 text-[13.5px] font-bold text-gray-900 dark:text-white leading-tight truncate">Backlog</span><ChevronRight size={16} className={`text-gray-400 dark:text-gray-500 shrink-0 transition-transform duration-200 ease-out ${backlogOpen ? 'rotate-90' : ''}`} /></span>
            <span className="flex items-center gap-2 mt-1 pl-[14px]"><span className="flex-1 min-w-0 text-[11px] text-gray-400 dark:text-gray-500 truncate">{backlog.length} pending · {backlog.length} lecture{backlog.length !== 1 ? 's' : ''}</span><span className="inline-flex items-center gap-1 px-1.5 py-px rounded-md border border-red-200 dark:border-red-500/30 text-[9px] font-bold tracking-wider text-red-500 dark:text-red-400 shrink-0"><span className="w-1 h-1 rounded-full bg-red-500" aria-hidden="true" />RED ZONE</span></span>
          </button>
          <div className={`grid transition-all duration-200 ease-out ${backlogOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}><div className="overflow-hidden"><div className="px-3 pb-3 pt-1 space-y-1.5 border-t border-gray-100 dark:border-white/5"><p className="px-1 pt-2 text-[10.5px] text-gray-400 dark:text-gray-500">Oldest lectures first</p>{backlog.map(l => <LectureCard key={l.id} lecture={{ ...l, isBacklog: true }} compact showDate showOriginal />)}</div></div></div>
        </div>
      )}

      {tomorrowLectures.length > 0 && (
        <div className="animate-fadeIn stagger-5">
          <h3 className="text-[14px] font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><ChevronRight size={16} className="text-purple-500" /> Up Next — {formatDateShort(tomorrow)} <span className="text-[11px] font-normal text-gray-400 dark:text-gray-500 ml-1">{tomorrowLectures.length} lecture{tomorrowLectures.length > 1 ? 's' : ''}{tomorrowLectures.some(l => l.isBacklog) && ' • backlog'}</span></h3>
          <div className="space-y-2">{tomorrowLectures.slice(0, 3).map(l => <LectureCard key={l.id} lecture={l} compact showDate />)}</div>
        </div>
      )}
      ...
      <div className="animate-fadeIn stagger-6 bg-white dark:bg-[#1a1c2b] rounded-2xl p-5 border border-gray-100 dark:border-white/5">
        <div className="flex items-center justify-between mb-3"><h3 className="text-[14px] font-semibold text-gray-900 dark:text-white">Overall Progress</h3><span className="text-[13px] font-bold text-indigo-600 dark:text-indigo-400">{overallPercent}%</span></div>
        <ProgressBar value={overallPercent} size="lg" showShimmer />
        <div className="flex justify-between mt-2 text-[11px] text-gray-500 dark:text-gray-400"><span>{totalCompleted} completed</span><span>{remaining} remaining • {Math.round(schedule.totalEffort || remaining)} effort units</span></div>
...
function StatCard({ icon, label, value, sub, color }) {
  return (
    <div className="card-surface rounded-xl p-4 card-hover transition-shadow duration-200">
      <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-white mb-3 shadow-lg`}>{icon}</div>
...
function SubjectMini({ label, total, completed, color }) {
  ...
  return (
    <div className="text-center">
      <div className="w-full h-1.5 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden mb-1.5"><div className={`h-full rounded-full ${color} transition-all duration-500`} style={{ width: `${pct}%` }} /></div>
